    ...
    beforeEach(async done => {
      ...
      try {
        ...
        mbConfig = await MbConfig.create({
          configKey: TEST_CONFIG.key,
          configValue: TEST_CONFIG.val
        });
    ...
    describe("When logged out", () => {
      it("can GET a config value by key from /mb-config/:key ", async done => {
        ...
        const response = await agent.get(
          "/api/v1/mb-config/" + TEST_CONFIG.key
        );
        // expect server to return config as JSON obj
        const jsonParsedTestConfigValue = JSON.parse(TEST_CONFIG.val);
  ...
  describe("PATCH /mb-config", () => {
    ...
    describe("When admin", () => {
      ...
      it("can PATCH /mb-config/:key", async done => {
        ...
        expect(response.body).toMatchObject(NEW_CONFIG_REQUEST);
