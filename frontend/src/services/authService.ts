...
export class AuthService {
  ...
  register(firstname: string, lastname: string, email: string, password: string): Promise<MbUser> {
    return this.apiService.post('/api/v1/auth/register', { firstname, lastname, email, password })
      .then(resp => resp.data);
  }
  logout(): Promise<any> {
